...
export async function POST(request) {
  ...
  try {
    ...
    // Check if this is a KlawKeeper email request (create mail account)
    let mailAccountCreated = false;
    
    // Consider any email during signup as a KlawKeeper email
    // You can change this if you only want to create mail accounts for specific domains
    const isKlawKeeperEmail = true; // For testing - create mail account for any email
    // const isKlawKeeperEmail = email.endsWith('@klawkeeper.xyz') || email.endsWith('@phoneshield.ai');
    ...
    const shouldCreateMailAccount = isKlawKeeperEmail; // Override environment check for testing
    ...
    if (shouldCreateMailAccount) {
      ...
      console.log(`[Register API] Skipping mail account creation. isKlawKeeperEmail=${isKlawKeeperEmail}, CREATE_MAIL_ACCOUNTS=${process.env.CREATE_MAIL_ACCOUNTS}`);
    }
